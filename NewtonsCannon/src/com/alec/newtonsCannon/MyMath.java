...
public class MyMath {
	...
	public static float scaleValue (float orig, float min, float max) {
		return scaleValue(orig, min, max, 0.0f, 1.0f);
	}
	
	public static float scaleValue (float orig, float min, float max, float nMin, float nMax) {
		return ( (nMax - nMin) * (orig - min) ) / (max - min) ;
	}
